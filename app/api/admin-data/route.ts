...
export async function GET(req: Request) {
  ...
  const [voters, candidates, votes, settings] = await Promise.all([
    supabase.from('voters').select('*').order('level').order('class_name').order('full_name').limit(3000),
    supabase.from('candidates').select('*').order('pair_number'),
    supabase.from('votes').select('*, voters(full_name, credential, level, class_name), candidates(pair_number, chair_name, vice_name)').order('created_at', { ascending: false }).limit(3000),
    supabase.from('site_settings').select('*'),
  ]);
  for (const res of [voters, candidates, votes, settings]) {
